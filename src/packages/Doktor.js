...
const Doktor = () => {
    ...
    function loadUsers() {
      fetch(`api/doktors?page=${currentPage-1}`)
          .then(res => res.json())
          .then((result) => {
    ...
    function deleteUser() {
      fetch(`/api/doktors/${selectedUser.id}`, {
          method: 'DELETE'
      }).then(() => {
          loadUsers();
          clearForm();
    ...
    <Container style={{backgroundColor: '#0C134F', color: 'white', borderRadius: '30px'} }>
      <Row>
        <Col sm={8}>
          ...
          <Col sm={4}>
            <br />
              <h1 className='text-center' style={{fontFamily:'Post No Bills Jaffna', fontSize:'60px'}}>Doktor Kayıt</h1>
                  <Form>
                      {errorMessage ? (
                      ...
                      <Form.Group>
                        ...
                        >
                            ...
                            <option value='HASTA' disabled>HASTA</option>

                        </Form.Select>
                      </Form.Group>
                      <br />
                      <Form.Group>
                      <Form.Label>Hastane Kimlik Numarası:</Form.Label>
                      <Form.Control
                        type="text"
                        autoComplete='off'
                        name="hastaneKimlik"
                        maxLength={'7'}
                        placeholder='Hastane Kimlik numarasını giriniz'
                        value={selectedUser.hastaneKimlik}
                        onChange={(e) => handleInputChange(e)}
                        className={selectedUser.role === 'HASTA' ? 'disabledRole' : ''}
                      />
                    </Form.Group>
                    <br />
                    <Form.Group>
